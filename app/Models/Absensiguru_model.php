...
class Absensiguru_model extends Model
{
    protected $table = 'tweb_pegawai_hadir';
    // Jika tidak ada ini, CI4 akan error
    protected $allowedFields = [
        'NO_INDUK', 'ID_PEGAWAI', 'TANGGAL', 'JAM', 'STATUS', 'STS',
        'NO_MESIN', 'LON', 'LAT', 'JAM_SETTING', 'KETERANGAN', 'KETERANGAN2',
        'FILENAME', 'SUHU', 'id_shift', 'sts_validasi', 'sts_nonlibur',
        'foto', 'longtitude', 'alasan_pulang'
    ];

    public function getAbsensi($id)
    {
        return $this->db->table($this->table)
        ->select('t_ptk.id_ptk as id_ptk,nomor_absensi,nip,nama_ptk,JAM,STATUS,nama_jenis_ptk')
        ->join('t_ptk', 't_ptk.id_ptk = tweb_pegawai_hadir.ID_PEGAWAI')
    ...
    public function getAbsensipulang($id)
    {
        return $this->db->table($this->table)
        ->select('t_ptk.id_ptk as id_ptk,nomor_absensi,nip,nama_ptk,JAM,STATUS,nama_jenis_ptk')
        ->join('t_ptk', 't_ptk.id_ptk = tweb_pegawai_hadir.ID_PEGAWAI')
    ...
    public function getAbsensitgluser($id, $tgl1, $tgl2)
    {
        return $this->db->table($this->table)
        ->where('TANGGAL >=', $tgl1)
        ->where('TANGGAL <=', $tgl2)
